...
/* Evita que una llamada a Firebase cuelgue la UI para siempre si la red
   está lenta/caída. Si no responde en `ms`, se rechaza con un error claro. */
function withTimeout(promise, ms, label){
  let timer;
  const timeout = new Promise((_, reject)=>{
    timer = setTimeout(()=>reject(Object.assign(new Error(`timeout:${label}`), {code:'auth/network-request-failed'})), ms);
  });
...
async function submitLogin(){
  ...
  try{
    ...
    if(_loginMode === 'register'){
      user = await withTimeout(window._firebase.signUp(email, pass), 15000, 'signUp');
    } else {
      user = await withTimeout(window._firebase.signIn(email, pass), 15000, 'signIn');
    }
...
window.onFirebaseUserReady = async function(user){
  ...
  try {
    cloudData = await withTimeout(window._firebase.loadData(user.uid), 12000, 'loadData');
  } catch(e){
  ...
  if(loadFailed){
    // No pudimos confirmar si hay datos en la nube (red lenta/caída).
    // NO tratamos esto como "usuario nuevo": eso sobrescribiría un progreso
    // real con un estado vacío. Seguimos con la sesión local y avisamos.
    hideLoadingOverlay();
    showToast('⚠️ No se pudo sincronizar con la nube. Revisa tu conexión.');
    applyTheme(state.theme||'dark');
    applyNativeLanguageUI();
    updateAllUI();
    routeAfterAuth();
    if(typeof silentlyReconcilePremium==='function') silentlyReconcilePremium();
