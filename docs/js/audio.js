...
async function speakMixedLanguageText(cleanText){
  ...
  // videollamada, sin contar el uso dos veces. IMPORTANTE: ya NO hay
  // reserva a la voz nativa del navegador (es de mala calidad); si no
  // hay cuota, simplemente no se reproduce voz hasta el reseteo diario o
  // hasta pasar a Premium.
  if(typeof canUseElevenTTS === 'function' && !canUseElevenTTS(cleanText.length)){
    if(!_ttsQuotaWarned){
      _ttsQuotaWarned = true;
      if(typeof showToast === 'function') showToast('🔊 Alcanzaste tu límite diario de voz. Hazte Premium para seguir escuchando a los personajes hoy.');
    }
    return;
  }

  if(typeof ttsFetchAudioBlob !== 'function') return;
  const wholeBlob = await ttsFetchAudioBlob(cleanText, cv, charId).catch(() => null);
  if(token !== _activeSpeechToken) return;
  if(!wholeBlob) return; // sin ElevenLabs disponible (sin key, caído, o falló la petición): sin voz, sin reserva nativa
...
async function speakText(rawText){
  ...
  // Sin ElevenLabs disponible (ni clave propia, ni gateway administrado
  // vía cuenta con IA gestionada), no hay voz — ya no se recurre a la
  // síntesis nativa del navegador/dispositivo, que suena muy mal.
