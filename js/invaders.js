...
const Invaders = (() => {
  ...
  let shipVideo = null, shipReady = false, shipCharId = null;

  const st = {
    running:false, paused:true,
    level:1, score:0, lives:3, hiScore:0, diff:0,
  };
  ...
  /* ── Video real de la nave ────────────────────────────────
     Antes la nave del jugador se dibujaba con ctx.fillText()
     usando un emoji. Ahora se dibuja el .webm real de la nave
     del personaje elegido, frame a frame, sobre el canvas —
     igual que ya se ve en el HUD y en el selector de piloto.
     Si el video no carga por alguna razón, cae de vuelta al
     emoji para que el juego nunca se rompa. */
  function _loadShipVideo(char) {
    if (!char || !char.ship) { shipVideo = null; shipReady = false; return; }
    if (shipCharId === char.id && shipVideo) return; // ya está cargada
    shipCharId = char.id;
    shipReady = false;
    ...
    v.playsInline = true;
    v.autoplay = true;
    v.addEventListener('loadeddata', () => { shipReady = true; });
    v.addEventListener('error', () => { shipReady = false; shipVideo = null; });
    v.play().catch(() => {});
    shipVideo = v;
  }
  ...
  function _update(dt) {
    ...
    if (moveTimer <= 0) {
      ...
      const step = Math.max(4, 13*cfg.spd*diffMult*(1+(30-alive.length)*0.02));
      let wall = false;
      ...
      alive.forEach(e => {
        e.sTimer -= 1;
        if (e.sTimer<=0 && Math.random()<cfg.freq*diffMult*alive.length) {
          bullets.push({ x:e.x+e.w/2-2, y:e.y+e.h, w:4, h:11, vy:(5+cfg.spd*0.8)*diffMult, player:false });
  ...
  function _draw(ts) {
    ...
    /* Nave del jugador — video real de la nave (.webm) dibujado
       frame a frame sobre el canvas. Si por algo no cargó, se
       usa el emoji del personaje como respaldo, nunca se rompe. */
    const char = GameSession.selectedChar;
    if (player.invTimer>0 && Math.floor(t*8)%2===0) ctx.globalAlpha=0.25;
    const equipped = typeof DrakonBridge!=='undefined' ? DrakonBridge.getInventory().equipped : {};
    const shipGlow = equipped.skin==='skin-legend'?'rgba(255,215,80,.9)':equipped.skin==='skin-flame'?'rgba(255,90,35,.9)':'rgba(255,192,105,.75)';
    ctx.shadowColor=shipGlow; ctx.shadowBlur=14;

    if (shipReady && shipVideo && shipVideo.readyState >= 2) {
      const w = player.w * 1.35, h = player.h * 1.35;
      const x = player.x + player.w/2 - w/2, y = player.y + player.h - h;
      ctx.drawImage(shipVideo, x, y, w, h);
    } else {
      ctx.font='28px serif';
      ctx.textAlign='center'; ctx.textBaseline='bottom';
