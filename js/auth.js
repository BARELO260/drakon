...
window.onFirebaseUserReady = async function(user){
  ...
  if(cloudData && Object.keys(cloudData).length > 0){
    ...
    // Guardar valores locales ANTES de pisar con la nube
    const localXP     = state.xp            || 0;
    const localStreak = state.streak         || 0;
    const localMsgs   = state.totalMessages  || 0;

    // Todos los campos no-acumulados: la nube gana siempre
    ...
    // Progreso acumulado: tomar siempre el mayor (evita perder XP por desincronización)
    ...
    // Si el local tenía más progreso, subir la versión fusionada a la nube de inmediato
    ...
    // Subir inmediatamente el estado local para registrar la cuenta en la nube.
    // Así el próximo login desde otro dispositivo encontrará datos y no se reinicia.
    const fbUser = window._fbUser || (window._fbAuth && window._fbAuth.currentUser);
    if(fbUser && window._firebase){
      window._firebase.saveData({ ...state, updatedAt: Date.now() });
    }
