...
window.onFirebaseUserReady = async function(user){
  ...
  if(cloudData && Object.keys(cloudData).length > 0){
    // Datos locales actuales (guardados en localStorage antes de pisar con nube)
    const localXP     = state.xp     || 0;
    const localStreak = state.streak  || 0;
    const localMsgs   = state.totalMessages || 0;

    // Campos que se sobreescriben directo desde la nube
    const KEYS = ['lastActiveDate','lastMsgDate','msgsToday','charId','lang','theme',
    ...
    // Para progreso acumulado: tomar el mayor valor entre local y nube
    // Evita que un dispositivo desincronizado pise el progreso real
    ...
    // Replicar al localStorage y subir el estado fusionado a Firestore
    try{ localStorage.setItem('drakon_pwa', JSON.stringify(state)); }catch(e){}
    loadGroqKey();

    // Si el local tenía más progreso que la nube, subir inmediatamente
    if(localXP > (cloudData.xp||0) || localStreak > (cloudData.streak||0)){
      const fbUser = window._fbUser || auth.currentUser;
