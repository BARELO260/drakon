...
/* ═══════════════════════════════════════
   SOUND FX (archivos de audio en assets/sounds/)
═══════════════════════════════════════ */
const SFX_FILES = {
...
function playSound(type){
  ...
  if(!src) return;
  try{
    // Reutilizamos un <audio> por tipo (evita crear uno nuevo en cada acierto/fallo)
    let audio = _sfxCache[type];
    if(!audio){
      audio = new Audio(src);
      audio.preload = 'auto';
      _sfxCache[type] = audio;
    }
    audio.currentTime = 0;
    audio.play().catch(()=>{}); // ignora bloqueo de autoplay del navegador si ocurre
  }catch(e){}
}
