...
/* ═══════════════════════════════════════
   MENU MUSIC — shuffle playlist, loops forever
═══════════════════════════════════════ */
const _MENU_TRACKS = [
  'assets/music/menu.mp3',
  'assets/music/menu1.mp3',
  'assets/music/menu2.mp3',
  'assets/music/menu3.mp3',
  'assets/music/menu4.mp3',
];
let _menuAudio = null;
let _menuPlaylist = [];
let _menuStarted = false;

function _shufflePlaylist(){
  const arr = [..._MENU_TRACKS];
...
function _playNextMenuTrack(){
  if(!_menuPlaylist.length) _menuPlaylist = _shufflePlaylist();
  const src = _menuPlaylist.shift();
  if(!_menuAudio){ _menuAudio = new Audio(); _menuAudio.volume = 0.35; }
  _menuAudio.src = src;
  _menuAudio.onended = _playNextMenuTrack;
  _menuAudio.play().catch(()=>{});
}

function startMenuMusic(){
  if(_menuStarted) return;
  _menuStarted = true;
  _menuPlaylist = _shufflePlaylist();
  _playNextMenuTrack();
}

function stopMenuMusic(){
  if(_menuAudio){ _menuAudio.pause(); _menuAudio.src=''; }
  _menuStarted = false;
}

(function(){
  document.addEventListener('pointerdown', function start(){ startMenuMusic(); document.removeEventListener('pointerdown', start); }, {once:true});
})();
