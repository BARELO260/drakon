...
const ClassicGames = (() => {
  ...
  function hangman() {
    ...
    state.word = shuffle(pool)[0]; state.used=[]; state.left=attempts; state.maxLeft=attempts;
    drawHangman();
  }
  function drawHangman() {
    const w=state.word, revealed=w.split('').map(c=>state.used.includes(c)?c:'_').join(' ');
    const wrong = Math.min(6, state.maxLeft - state.left);
    const letters='ABCDEFGHIJKLMNÑOPQRSTUVWXYZ'.split('');
    host().innerHTML=`<div class="classic-center"><div class="hang-figure">${['◯','│','╱','╲','╱','╲'].slice(0,wrong).join(' ')||'⌁'}</div><div class="hang-word">${revealed}</div><p>Intentos restantes: <b>${state.left}</b></p><div class="letter-grid">${letters.map(l=>`<button ${state.used.includes(l)?'disabled':''} onclick="ClassicGames.letter('${l}')">${l}</button>`).join('')}</div></div>`;
  }
  function letter(l) { if (state.used.includes(l)) return; state.used.push(l); if(!state.word.includes(l)) state.left--; if(state.word.split('').every(c=>state.used.includes(c))){add(120+level()*40+state.left*15); return finish(`¡Encontraste “${state.word}”!`);} if(!state.left){ GamesCore.toast(`La palabra era ${state.word}`); return setTimeout(hangman,1000); } drawHangman(); }
  ...
  /* Los símbolos Unicode "blancos" (♔♕♖♗♘♙) son huecos por diseño de
     fuente: solo dibujan el contorno, así que el fondo de la casilla
     se ve A TRAVÉS de la pieza. Sobre una casilla oscura eso hace que
     una pieza blanca "parezca negra". Para evitarlo, SIEMPRE se
     dibuja con el glifo sólido/relleno (el mismo que usan las piezas
     negras) y el color de equipo se aplica por CSS (w-piece/b-piece),
     no por el propio carácter. El tablero interno (state.board) sigue
     usando los caracteres originales para toda la lógica del juego. */
  const SOLID_GLYPH = {'♙':'♟','♖':'♜','♘':'♞','♗':'♝','♕':'♛','♔':'♚','♟':'♟','♜':'♜','♞':'♞','♝':'♝','♛':'♛','♚':'♚'};
  ...
  function animatePieceMove(sr,sc,r,c,piece,onDone){
    ...
    fly.className = `chess-fly-piece ${isWhite ? 'w-piece' : 'b-piece'}`;
    fly.textContent = SOLID_GLYPH[piece] || piece;
  ...
  return {start,replay,chooseChessMode,startChess,letter,cell,clearSelection,checkSudoku,stock,move,checkKakuro,chessMove,openThemePicker,closeThemePicker,setChessTheme};
})();
