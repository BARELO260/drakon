...
/* La definición de SITUATIONS y toda la lógica de la sección "Situaciones"
   ...
   Este módulo (chat.js) conserva solo lo que pertenece al chat de IA en
...
${coreRules}
SITUATION STAGE: PRACTICE ROLEPLAY — you are playing the role of ${sit.role||'a local person in this situation'}. Stay fully in character as that person (never break character to sound like a generic assistant); run the interaction naturally one exchange at a time, and increase difficulty as the learner succeeds.
EXTRA FOR SITUATIONS: Respond MAINLY in ${native}, staying in character as ${sit.role||'the person in this scene'}. Do not interrupt constantly to correct every mistake — let the conversation flow naturally; correct only occasional important errors, briefly. After each exchange add [Useful vocab: word = translation in ${nativeLangName}]. Only correct errors in ${native}.${typeof memoryPrompt==='function'?memoryPrompt():''}`;
  }
...
function _launchChat(mode,sit){
  ...
  goTo('screen-chat');
  if(typeof mascotReset==='function') mascotReset();
  // Puesta en escena: si es una situación, el fondo de la situación y la
  // mascota se integran visualmente (ver js/situations.js). Si no, se
  // limpia cualquier fondo que hubiera quedado de una sesión anterior.
  if(typeof applySituationChatScene==='function') applySituationChatScene(mode==='situation'?sit:null);
  ...
  const welcomes={
    ...
    situation:`¡Perfecto! Seré ${sit?.roleLabel||'tu interlocutor'} en: **${sit?.name}**. Comienza en ${lang} cuando quieras. 🎭`,
  };
