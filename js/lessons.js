...
/* ════════════════════════════════════════════════════════════
   BANCO DE LECCIONES — INGLÉS PRÁCTICO (formato compacto)
   Niveles: A1 · A2 · B1 · B2

   ── CÓMO AGREGAR LECCIONES NUEVAS RÁPIDO ──
   Cada lección es un objeto con: id, level, title, emoji, xp, description
   y "ex" (ejercicios). Cada ejercicio es un ARRAY posicional, no un objeto,
   para escribir muchas lecciones rápido sin repetir keys:

     [tipo, pregunta, [op1,op2,op3,op4], índiceCorrecto, explicación, contexto?]

   - tipo: 'mcq' | 'translate' | 'fill' | 'arrange'  (solo es una etiqueta
     visual/descriptiva — TODOS usan 4 opciones + índice correcto).
   - contexto es OPCIONAL: si no aplica, simplemente omite el 6º elemento.
   - SIEMPRE deben ser exactamente 4 opciones.

   Ejemplo de una lección completa:

     {
       id:'a1_ejemplo', level:'A1', title:'Mi lección', emoji:'✨', xp:30,
       description:'Descripción corta de la lección.',
       ex:[
         ['mcq', '¿Pregunta?', ['A','B','C','D'], 0, 'Por qué A es correcta.', 'Contexto opcional.'],
         ['fill', 'Completa: "I ___ happy"', ['am','is','are','be'], 0, 'Explicación...'],
       ]
     },

   Para agregar 20-30 lecciones de una sola vez, lo más rápido es pedirle
   a Claude que las genere directamente en este formato para el nivel y
   tema que quieras (ej. "genera 10 lecciones B1 sobre viajes").
   ════════════════════════════════════════════════════════════ */
...
const ENGLISH_LESSONS = buildLessons(LESSON_SOURCE);
...
const LessonEngine = {
  ...
  answer(selectedIdx) {
    ...
    // Sonido
    if (typeof state !== 'undefined' && state.sounds) {
...
function renderLessons() {
  ...
  console.log('[Drakón][lessons] renderLessons() ejecutado. Contenedor encontrado:', !!container, '· Lecciones totales:', (typeof ENGLISH_LESSONS!=='undefined'?ENGLISH_LESSONS.length:'ENGLISH_LESSONS no definido'));
  if (!container) return;

  const completedIds = (typeof state !== 'undefined') ? (state.lessonsCompleted || []) : [];
  const totalLessons = ENGLISH_LESSONS.length;
  const totalDone = ENGLISH_LESSONS.filter(l => completedIds.includes(l.id)).length;
  const totalXp = ENGLISH_LESSONS.filter(l => completedIds.includes(l.id)).reduce((s,l)=>s+l.xp,0);
